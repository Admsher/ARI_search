...
public class TestReadSpeed {
    ...
    private static void doRead() {
        ...
        int characters = 0;
        int words = 0;        
        File file = new File("nytsample/NYT_00193153.CSV");
        ArticleIndex articles = new ArticleIndex(5, 10000);
        ...
        CSVParser csvParser = new CSVParser(reader, CSVFormat.DEFAULT.builder().setHeader().setSkipHeaderRecord(true).build())) {
            ...
            for (CSVRecord record : csvParser) {
                ...
                characters += articleToAdd.length();
                words += articleToAdd.replaceAll("[^a-zA-Z0-9\\s++]", "").toLowerCase().split("\\s+").length;
            ...
            System.out.println(articles.NumberOfArticles() + "," + Utilities.sizeof(articles) + "," + Utilities.sizeof(articles)/articles.NumberOfArticles() + "," + characters + "," + words);
        }
    ...
    public static void main(String[] args) {
        int[] integerArray = IntStream.range(0,10000).toArray();

        System.out.println("Length of an Integer is " + Utilities.sizeof(integerArray) );
        for (int i = 0; i < 10; i++) {
            doRead();
        }
